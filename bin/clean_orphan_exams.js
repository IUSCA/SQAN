...
db.init(function(err) {
  ...
  db.Exam.find({}).populate('research_id').exec(function(err, _exams) {
    _exams.forEach(function(_exam) {
      // console.log(`Checking exam ${_exam._id}`);
      if(_exam.research_id === null) {
        console.log(`Exam ${_exam._id} is an orphan!`);
      } else {
        console.log(_exam.research_id.IIBISID);
        console.log(_exam.research_id.StationName);
      }
    })
  })
